  ...
  methods: {
    ...
    beginFlooding() {
      if (this.sortGroupGrids) {
        // this.intervalSetting(this.sortGroupGrids)
        this.drawPrimitives(this.features)
      }
    },
    pauseFlooding() {
      //清楚计时器
      ...
      this.features.forEach(v => {
        let id = v.properties.GridID,
          attributes = this.primitive.getGeometryInstanceAttributes(id);
        attributes.color = Cesium.ColorGeometryInstanceAttribute.toValue(Cesium.Color.RED, attributes.color);
      })
    ...
    toggle(name) {
      ...
      if (name == "openFloodPanel") {
        ...
        if (!this.sortGroupGrids) {
          ...
          for (let i = 0; i < 64; i++) {
            this.sortGroupGrids[i].forEach(v => {
    ...
    drawalls() {
      let features = []
      for (let i = 0; i < 65; i++) {
        sortGroupGrids[i].forEach(v => {
          features.push(v)
        })
        this.drawPrimitives(features)
      }
    },
    reset() {
      //删除primitives
